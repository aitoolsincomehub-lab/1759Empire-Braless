...
export default function Concierge() {
  ...
  return <div className="concierge"><button className="conciergeTrigger" onClick={openPanel} aria-expanded={open} aria-label="Open AI Concierge">1759 <span>AI Concierge</span></button>{open && <section className="conciergePanel" aria-label="AI Concierge"><header><div><p className="eyebrow">1759 EMPIRE · AI CONCIERGE</p><h2>Ask about 1759</h2><p>Ask about rooms, dining, events and planning your visit.</p></div><button className="conciergeClose" onClick={() => setOpen(false)} aria-label="Close AI Concierge">×</button></header><div className="conciergeMessages">{messages.map((message, index) => <div className={`conciergeMessage ${message.role}`} key={`${message.role}-${index}`}><p>{message.content}</p>{message.actions?.map((action) => <Link href={action.href} className="conciergeAction" key={`${action.type}-${action.href}`} onClick={() => { trackEvent("ai_concierge_cta_clicked", { cta_type: action.type, page: window.location.pathname }); if (action.type === "WHATSAPP") trackEvent("whatsapp_cta_clicked", { cta_location: "concierge", page: window.location.pathname }); }}>{action.label} →</Link>)}</div>)}{loading && <div className="conciergeMessage assistant"><p>Checking the right information...</p></div>}</div><div className="conciergeStarters">{messages.length === 1 && starters.map((starter) => <button key={starter} onClick={() => send(undefined, starter)}>{starter}</button>)}</div><form className="conciergeForm" onSubmit={send}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask about 1759..." aria-label="Ask the AI Concierge" /><button className="button" disabled={loading || !input.trim()}>Send</button></form></section>}</div>;
}
