...
export default function AdminEnquiries({ initialEnquiries }: { initialEnquiries: EnquiryRow[] }) {
  ...
  async function update(id: string, status: EnquiryStatus) {
    try {
      ...
      const result = await response.json().catch(() => ({})) as { ok?: boolean; data?: EventReservation; error?: string };
      if (!response.ok || !result.ok) setMessage(result.error || "We couldn't update that enquiry. Please try again.");
